...
class Chord {
  ...
  public static parse(token: string): Chord {
    const result = XRegExp.exec(token, CHORD_REGEX);

    if (!result) {
      throw new Error(`Token ${token} cannot be parsed`);
    }
...
const CHORD_REGEX = (XRegExp as any)(
  `^${ROOT_PATTERN}${SUFFIX_PATTERN}${BASS_PATTERN}$`
);
const MINOR_CHORD_REGEX = (XRegExp as any)(
  `^${ROOT_PATTERN}${MINOR_PATTERN}.*$`
);

/** Fluent API for transposing text containing chords. */
class Transposer {
  public tokens: Array<Array<Chord | string>>;
  public currentKey?: KeySignature;

  /**
   * Creates a new Transpose instance
   * @param text
   */
  public static transpose(text: string | Array<Array<Chord | string>>) {
   ...
   * @param text
   */
  constructor(text: string | Array<Array<Chord | string>>) {
    ...
    for (let line of this.tokens) {
      for (let token of line) {
        if (token instanceof Chord) {
          return KeySignatures.valueOf(
            token.root + (MINOR_CHORD_REGEX.test(token.toString()) ? "m" : "")
          );
        }
      }
    }
  ...
  public fromKey(key: string | KeySignature): Transposer {
    this.currentKey =
      key instanceof KeySignature ? key : KeySignatures.valueOf(key);
    return this;
  }

  public up(semitones: number): Transposer {
  ...
  public down(semitones: number): Transposer {
    return this.up(-semitones);
  }

  public toKey(toKey: string): Transposer {
    const key = this.getKey();
...
function tokenize(
  text: string,
  threshold?: number
): Array<Array<Chord | string>> {
  if (threshold === undefined) {
    threshold = 0.5;
  }
  const lines: string[] = text.split("\n");

  const newText: Array<Array<Chord | string>> = [];
...
function transposeTokens(
  tokens: Array<Array<Chord | string>>,
    ...
    line.forEach((token, i) => {
      if (typeof token === "string") {
        ...
        const transposedChord = new Chord(
          transpositionMap.get(token.root) as string,
          token.suffix,
          transpositionMap.get(token.bass as string)
        );
